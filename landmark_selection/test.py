...
def get_image_from_anndata(adata: ad.AnnData,
...
def read_data(path: str)->Tuple[Optional[ad.AnnData],Image.Image,float]:
    ...
    if any([path.endswith(ext) for ext in image_ext]):
        ...
        image = image.resize((int(im_w * scalefactor),int(im_h * scalefactor)),Image.ANTIALIAS)
        scalefactor = 1 / scalefactor
        adata = None
        ...
        print("Data format not supported. Exiting.")
        sys.exit(-1)
...
class MainWindow(QMainWindow):
    ...
    def mousePressEvent(self, QMouseEvent):
        pos = QMouseEvent.pos()
        if pos.y() <= self.im_h + self.margin and\
           pos.y() >= self.margin and\
           pos.x() <= self.im_w + self.margin and\
           pos.x()>= self.margin:
    ...
    def UiComponents(self):
        ...
        undo_button.setGeometry(self.w / 2 - undo_button_w - button_margin/2 ,
                           self.im_h + 50,
                           undo_button_w,
                           undo_button_h)
        ...
        save_button.setGeometry(self.w / 2 + button_margin/2,
                           self.im_h + 50,
                           save_button_w,
                           save_button_h)
        ...
        if self.adata is None:
            ...
            self.s_tf.setGeometry(self.w / 2 - tf_margin / 2 - s_tf_w,
                            tf_y, 
                            s_tf_w,
                            s_tf_h,
                            )
            ...
            self.o_tf.setGeometry(self.w / 2 + tf_margin / 2,
                            tf_y, 
                            o_tf_w,
                            o_tf_h,
                            )
        ...
        # self.outdir = o_tf.text()
        # self.sample = s_tf.text()

    def _update_outdir(self,):
    ...
    def savepoints(self,):
        ...
        if self.adata is None:
            ...
            basename = osp.join(self.outdir,
                                self.sample + "_landmarks")
